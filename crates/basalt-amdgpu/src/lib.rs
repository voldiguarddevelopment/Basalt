// basalt-amdgpu: hand-rolled RDNA3 (gfx1100) instruction encoder.
//
// This crate is encoder primitives only, the AMDGCN equivalent of `basalt-x86/src/enc.rs`:
// one Rust function per real instruction *form*, each producing exact machine-code bytes.
// It does not decide how to lower BIR into instruction sequences (that is a later, separate
// crate module) and it is not a general-purpose AMDGPU assembler — only the subset of
